...
///////////////////////////////////////////////////
    ...
    public boolean add(E value, int frequency) {
        if (root == null) {
            root = new BinaryNode<E>(value, frequency);
            return true;
        }
        
        return addHelper(value, root, frequency);
    }

    public boolean addHelper(E value, BinaryNode<E> currentPos, int frequency) {


        if (frequency > currentPos.getFrequency()) {
            if (currentPos.hasRight()) {
                return addHelper(value, currentPos.getRight(), frequency);
            } else {
                BinaryNode<E> newNode = new BinaryNode<E>(value, frequency);
                currentPos.setRight(newNode);
                newNode.setParent(currentPos);
            ...
            if (currentPos.hasLeft()) {
                return addHelper(value, currentPos.getLeft(), frequency);
            } else {
                BinaryNode<E> newNode = new BinaryNode<E>(value, frequency);
                currentPos.setLeft(newNode);
                newNode.setParent(currentPos);
